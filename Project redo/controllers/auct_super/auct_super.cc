...
private:
  ...
  void markEventsDone(event_queue_t& event_queue) {
    for (auto& event : events_) {
      // Debug: Check the state of each event
      printf("Checking Event %d: Finished=%d, Active=%d\n", 
              event->id_, event->is_done(), num_active_events_);
      // If the event is not finished, add it to the queue
      if (!event->is_done()) {
          event_queue.emplace_back(event.get(), MSG_EVENT_NEW);
          printf("Added Event %d to the event_queue. (Type: %s, Pos: [%.2f, %.2f])\n",
                  event->id_, event->type_ == A ? "A" : "B",
                  event->pos_.x, event->pos_.y);
      } else {
        if (event->processed_ == 0) {
          num_events_handled_++;
  ...
  bool step(uint64_t step_size) {
    ...
    for (int i=0;i<NUM_ROBOTS;i++) {  
      ...
      if (wb_receiver_get_queue_length(receivers_[i]) > 0) {
        ...
        r_msg = (robot_message_t*) wb_receiver_get_data(receivers_[i]); 
