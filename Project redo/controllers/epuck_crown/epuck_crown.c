...
void update_active_tasks(int target_list_length)
{
    int k = 0;
    for (int j=0; j<99; j++) {
        if (all_tasks[j].is_completed == 0) {
            ...
            for (int l=0; l<NUM_ROBOTS; l++) {
                active_tasks[k].bid_values[l] = INFINITY;
            }
            k += 1;
            continue;
        }
...
static void receive_updates() 
{
   //printf("Robot %d receiving updates now.\n", robot_id);
    ...
    printf("It is working until now.\n");
    while (wb_receiver_get_queue_length(receiver_tag) > 0) {
        ...
        */
        ...
        //printf("Message robot id : %d, robot it : %d\n", msg.robot_id, robot_id);
        ...
        update_active_tasks(target_list_length);
    }
...
void reset(void) 
{
    ...
    printf("  Emitter: %d\n", emitter_tag);
    printf("  Receiver: %d\n", receiver_tag);
    printf("  Short-Range Emitter: %d\n", sr_emitter_tag);
    printf("  Short-Range Receiver: %d\n", sr_receiver_tag);
...
void receive_task_info() {
    const robot_robot_message_t *msg;
    ...
    while (wb_receiver_get_queue_length(sr_receiver_tag) > 0) {
        ...
        msg = (robot_robot_message_t*) wb_receiver_get_data(sr_receiver_tag);
        ...
        for (int j = 0; j < NUM_ROBOTS; j++) {
            ...
            //active_tasks[msg->task_id].is_completed = msg->is_completed;
