  /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * file:        epuck_distributed.c
 * author:      
 * description: E-puck file for distributed market-based task allocations (DIS lab05)
 *
...
#define SHORT_COM_RANGE 0.3 // Communication range in m
#define F_M 1000/TIME_STEP // Fm - maximum message transmission rate
#define RANGE 1024/2
#define BASELINE 500
#define V_MAX 0.5 // v_max - maximum robot velocity (m/s)
...
double weights[4] = {0.1, 0.4, 0.45, 0.5}; // weight of each velocity term
double speed[2]; // wheel speeds
double prev_speed[2]; // wheel speeds
uint16_t robot_id;

static WbDeviceTag emitter, receiver; // Radio
WbDeviceTag left_motor; // handler of left wheel of the robot
WbDevicetag right_motor; // handler of right wheel of the robot
WbDeviceTag turret_sensor;
WbDeviceTag distance_sensor[NB_SENSORS];
WbDeviceTag gps;

double braitenberg_coefficients[NB_SENSORS][2] = 
    { {152, 35}, {100, -15}, {80, -10}, {-10, -10},
...
static void receive_updates() 
{
    ...
    while (wb_receiver_get_queue_length(receiver_tag) > 0) {
        ...
        // channel = robot_id + 1, channel 0 reserved for physics plugin
        ...
        {    
            ...
            // Send bid to neighboring robots
...
void run(int ms)
{
    // Variables coming from lab 06 on mobile network
    int i, j , N, count = 0;
    double perf_data[3], buffPB[3];
    double ps_value[NB_SENSORS];
    double clock = 0;
