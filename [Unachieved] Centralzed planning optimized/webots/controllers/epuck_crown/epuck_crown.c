...
#define INFINITY_D          1000000
#define INFINITY_E          100000000.2
#define DEBUG 1
...
double last_bid = 0.0;
double time_since_last_bid = 0.0;
...
float dist_float(double x0, double y0, double x1, double y1) {
    return sqrt((x0-x1)*(x0-x1) + (y0-y1)*(y0-y1));
}

float compute_single_cost(double task_x,double task_y, double current_pos_x, double current_pos_y, Event_type event_type){
    //double distance_to_task = dist(my_pos[0], my_pos[1], msg.event_x, msg.event_y);
    float distance_to_task = dist_float(current_pos_x,  current_pos_y, task_x,task_y);

    float time_to_target = distance_to_task*10000 / MAX_SPEED;
    float time_to_handle_task = 0;
    ...
    float total_time = time_to_target + time_to_handle_task;
...
float compute_cost(event_data event_data_list[3], int i, int j, int k){
    ...
    cost += compute_dist(my_position, event_data_list[indices[0]]) / MAX_SPEED;
    cost += compute_delay(event_data_list[indices[0]], robot_id);

    for (int m = 1; m < 3; m++){
        cost += compute_dist(event_data_list[indices[m-1]], event_data_list[indices[m]]) / MAX_SPEED;
        cost += compute_delay(event_data_list[indices[m]], robot_id);
    }
    return cost;
}

float compute_best_cost(int target_list_length, double event_x, double event_y, Event_type event_type){
    switch (target_list_length){
        case 0:
            return compute_single_cost(event_x, event_y, my_pos[0], my_pos[1], event_type);
            break;
        case 1:
            float cost_1 = compute_single_cost(event_x, event_y, my_pos[0], my_pos[1], event_type)+ compute_single_cost(event_x, event_y, target[0][0], target[0][1], target[0][3]);
            float cost_2 = compute_single_cost( my_pos[0], my_pos[1], target[0][0], target[0][1], target[0][3]) + compute_single_cost( event_x, event_y, target[0][0], target[0][1], event_type);
            if ( cost_1 < cost_2)return cost_1;
            else return cost_2;
            break;
        case 2:
        event_data event_data_list[3];
        event_data_list[0] = (event_data){target[0][0], target[0][1], target[0][3]};
        event_data_list[1] = (event_data){target[1][0], target[1][1], target[1][3]};
        event_data_list[2] = (event_data){event_x, event_y, event_type};
        float best_cost = -1.0;
        float cost = 0.0;
        for (int i = 0; i < 3; ++i) {
                for (int j = 0; j < 3; ++j) {
...
void rearange_target(int target_list_length){
    switch (target_list_length)
    {
    case 2:
        float cost_1 = compute_single_cost(target[0][0], target[0][1], my_pos[0], my_pos[1], target[0][3])+ compute_single_cost(target[1][0], target[1][1], target[0][0], target[0][1], target[1][3]);
        float cost_2 = compute_single_cost( my_pos[0], my_pos[1], target[1][0], target[1][1], target[1][3]) + compute_single_cost(target[1][0], target[1][1], target[0][0], target[0][1], target[0][3]);
        if ( cost_1 > cost_2){
            float bx = target[0][0];
            float by = target[0][1];
            float btype = target[0][3];
    ...
    case 3:
        ...
        event_data_list[0] = (event_data){target[0][0], target[0][1], target[0][3]};
        event_data_list[1] = (event_data){target[1][0], target[1][1], target[1][3]};
        event_data_list[2] = (event_data){target[2][0], target[2][1], target[2][3]};
        float best_cost = -1.0;
        float cost = 0.0;
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 3; ++j) {
        ...
        for (int i = 0; i < 3; ++i) {
            target[i][0] = event_data_list[best_arrangement[i]].x;
            target[i][1] = event_data_list[best_arrangement[i]].y;
            target[i][3] = event_data_list[best_arrangement[i]].type;
        }
        break;
    }
}
...
static void receive_updates() 
{
    ...
    while (wb_receiver_get_queue_length(receiver_tag) > 0) {
        ...
        else if(msg.event_state == MSG_QUIT)
        {
            ...
            state = HANDLING_TASK;
            last_bid -= time_since_last_bid;

            time_since_last_bid = -compute_delay((event_data){msg.event_x, msg.event_y, msg.event_type}, robot_id);
        }
        ...
        else if(msg.event_state == MSG_EVENT_WON)
        {
            ...
            log_message("indx %d", indx);
            float total_time = compute_best_cost(target_list_length, msg.event_x, msg.event_y, msg.event_type);
            last_bid = total_time;
            ...
            target[msg.event_index][3] = msg.event_type == A ? 0 : 1;
            

            rearange_target(target_list_length);
            time_since_last_bid = 0;
        ...
        {                
            ...
            indx = target_list_length;
            float total_time = compute_best_cost(target_list_length, msg.event_x, msg.event_y, msg.event_type); // best time to do all task 

            // Total energy required to reach and handle the task + 10% buffer
            double required_energy = total_time * 1.1;
            ...
            if (energy_level >= required_energy) {
                ...
                if(indx == 0){
                    ...
                    bid = total_time/(indx+1);
                }
                else
                {
                    bid = INFINITY_D;
                }
                const bid_t my_bid = {robot_id, msg.event_id, bid, indx};

                wb_emitter_set_channel(emitter_tag, robot_id+1);
                wb_emitter_send(emitter_tag, &my_bid, sizeof(bid_t));  
                log_message("[DEBUG] Total time: %.2f, Last bid: %.2f, Time since last bid: %.2f, Calculated bid: %.2f\n", 
                    total_time, last_bid, time_since_last_bid, bid);

            }else{
                const bid_t my_bid = {robot_id, msg.event_id, INFINITY_E, indx};
                wb_emitter_set_channel(emitter_tag, robot_id+1);
                wb_emitter_send(emitter_tag, &my_bid, sizeof(bid_t));  
            }
        }
...
void compute_go_to_goal(int *msl, int *msr) 
{
    ...
    float Ku = 0.6;    // Forward control coefficient
