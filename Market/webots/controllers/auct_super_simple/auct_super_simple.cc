 ...
 * $Date$
 * $Author$   Last update on March 2022 by Wanting Jin
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
...
#define NUM_ACTIVE_EVENTS 2          // number of active events
#define TOTAL_EVENTS_TO_HANDLE  2   // Events after which simulation stops or...
...
class Supervisor {
  ...
  void addEvent() {
    ...
    Point2d pos_;
    if(next_event_id_==0){
      pos_.x = -0.4; pos_.y = 0.1;
    }
    if(next_event_id_==1){
      pos_.x = -0.4; pos_.y = -0.75;
    }
    //SIMPLE WORLD
    events_.push_back(unique_ptr<Event>(new Event{next_event_id_++, pos_})); // add to list
  ...
  bool step(uint64_t step_size) {
    ...
    for (int i=0;i<NUM_ROBOTS;i++) {
      ...
      if (wb_receiver_get_queue_length(receivers_[i]) > 0) {
        ...
        printf("robot %d placed a bid of %f for event %d \n", pbid->robot_id, pbid->value, pbid->event_id);
